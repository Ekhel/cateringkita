...
$order_nonmember = $this->db->get_where('temporary_order', ['read_order' => 0, 'order_status' => 'pending', 'order_toko' => $admin['toko']])->result_array();
$order = $this->db->get_where('order', ['read_order' => 0, 'order_status' => 'pending', 'is_order_hapus' => '1', 'order_toko' => $admin['toko']])->result_array();
...
$dataTampunganRole = [];
$TampunganStockMember = [];
$TampunganStockNonMember = [];
...
if ($order) {
    foreach ($order as $ord) {
        ...
        $this->db->where(['read_order' => 0, 'order_status' => 'pending', 'is_order_hapus' => '1', 'order.kd_order' => $ord['kd_order']]);
        $data = $this->db->get()->result_array();

        foreach ($data as $dt) {
            if ($dt['jns_produk'] == "stock") {
                array_push($TampunganStockMember, $dt['kd_order']);
            }
        }

        if (count($data) == count($TampunganStockMember)) {
            $this->db->select("*");
            $this->db->from("order");
            $this->db->join("detail_order", "order.kd_order=detail_order.kd_order");
            $this->db->join("produk", "produk.kd_produk=detail_order.kd_produk");
        ...
        foreach (array_keys($TampunganStockMember, $ord['kd_order']) as $key) {
            unset($TampunganStockMember[$key]);
        }
    }
} else {
    array_push($AngkaNotifMember, 0);
...
if ($order_nonmember) {
    foreach ($order_nonmember as $ord) {
        ...
        $this->db->where(['read_order' => 0, 'order_status' => 'pending', 'temporary_order.kd_temporary_order' => $ord['kd_temporary_order']]);
        $data = $this->db->get()->result_array();

        foreach ($data as $dt) {
            if ($dt['jns_produk'] == "stock") {
                array_push($TampunganStockNonMember, $dt['kd_temporary_order']);
            }
        }
        // var_dump(count($data));
        // var_dump(count($TampunganStockNonMember));
        // echo "<br>";

        if (count($data) == count($TampunganStockNonMember)) {
            $this->db->select("*");
            $this->db->from("temporary_order");
            $this->db->join("temporary_detail_order", "temporary_order.kd_temporary_order=temporary_detail_order.kd_temporary_order");
            $this->db->join("produk", "produk.kd_produk=temporary_detail_order.kd_produk");
        ...
        // untuk menghapus data push array di data yang sebelum nya
        foreach (array_keys($TampunganStockNonMember, $ord['kd_temporary_order']) as $key) {
            unset($TampunganStockNonMember[$key]);
        }
    }
} else {
    array_push($AngkaNotifNonMember, 0);
